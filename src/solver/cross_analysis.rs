...
impl CrossAnalyzer {
    ...
    fn edge_forcing_column(
        ...
        for (row, &cell) in column.iter().enumerate() {
            if cell == CellState::Filled && row < first_block {
        ...
        for row in (0..length).rev() {
            if column[row] == CellState::Filled && row >= length - last_block {
...
mod tests {
    ...
    fn test_overlap_analysis_simple() {
        ...
        assert!(deductions
            .iter()
            .any(|d| d.col == 2 && d.state == CellState::Filled));
        assert!(deductions
            .iter()
            .any(|d| d.col == 3 && d.state == CellState::Filled));
        assert!(deductions
            .iter()
            .any(|d| d.col == 4 && d.state == CellState::Filled));
    }
    ...
    fn test_edge_forcing() {
        ...
        assert!(deductions
            .iter()
            .any(|d| d.col == 0 && d.state == CellState::Filled));
        assert!(deductions
            .iter()
            .any(|d| d.col == 2 && d.state == CellState::Filled));
    }
