use crate::grid::{CellState, Constraints};
use crate::image_parser::{ImageParser, ParserConfig};
...
impl AdvancedConstraintExtractor {
    ...
    pub fn extract_auto(image: &DynamicImage) -> Result<Constraints, String> {
        ...
        Self::extract_from_regions(image, row_region, col_region, grid_width, grid_height)
    }

    /// Détecte automatiquement la grille dans l'image
    pub fn detect_grid(image: &DynamicImage) -> Result<(u32, u32, u32, usize, usize), String> {
        ...
        if horizontal {
            ...
            for x in 0..width {
                ...
                if dark_pixels as f32 / height as f32 > 0.5
                    && (lines.is_empty() || x as usize - lines[lines.len() - 1] > 5)
                {
    ...
    pub fn extract_from_regions(
        ...
        #[cfg(not(feature = "ocr"))]
        {
            ...
            Err(
                "La fonctionnalité OCR n'est pas activée. Recompilez avec --features ocr"
                    .to_string(),
            )
        }
    }
    ...
    pub fn extract_from_image_heuristic(image: &DynamicImage) -> Result<Constraints, String> {
        // Détecter la grille
        let (_grid_x, _grid_y, _cell_size, grid_width, grid_height) = Self::detect_grid(image)?;
    ...
    /// Extrait les contraintes en analysant les cases remplies de la grille
    /// Fonctionne pour les grilles pré-remplies où la solution est visible
    pub fn extract_constraints_from_filled_cells(
        image: &DynamicImage,
        ...
        // Parser l'image pour extraire l'état des cases
        let parser = ImageParser::new(parser_config.clone());
        let grid = parser
            .parse_image(image, grid_width, grid_height)
            .map_err(|e| format!("Erreur lors du parsing de la grille: {}", e))?;

        // Extraire les contraintes depuis les cases remplies
        let mut row_constraints = Vec::new();
        for row in 0..grid_height {
            let row_data = grid.get_row(row).unwrap();
            let constraints = Self::extract_line_constraints(&row_data);
            row_constraints.push(constraints);
        }

        let mut col_constraints = Vec::new();
        for col in 0..grid_width {
            let col_data = grid.get_column(col).unwrap();
            let constraints = Self::extract_line_constraints(&col_data);
            col_constraints.push(constraints);
        }

        Constraints::new(grid_width, grid_height, row_constraints, col_constraints)
    }

    /// Extrait les contraintes d'une ligne (ou colonne) en comptant les suites de cases remplies
    fn extract_line_constraints(line: &[CellState]) -> Vec<usize> {
        let mut constraints = Vec::new();
        ...
        // Ne pas oublier la dernière suite
        if current_run > 0 {
            constraints.push(current_run);
        }
